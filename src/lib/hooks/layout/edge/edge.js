...
export const isHorizontalFromPosition = (position) => {
 ...
 */
export const getLineCenter = (p1, p2) => {
    return {
        id: uuid(),
        x: (p1.x + p2.x) / 2,
        y: (p1.y + p2.y) / 2,
 ...
 */
export const isLineContainsPoint = (start, end, p) => {
    return (
        (start.x === end.x &&
            p.x === start.x &&
 ...
 *
 */
export function getPathWithRoundCorners(points, radius) {
    if (points.length < 2) {
        throw new Error('At least 2 points are required.');
    }

    function getRoundCorner(center, p1, p2, radius) {
        const {x, y} = center;
        ...
        if (isHorizontal) {
            return `L ${x + radius * xDir},${y}Q ${x},${y} ${x},${
                y + radius * yDir
            }`;
        }

        return `L ${x},${y + radius * yDir}Q ${x},${y} ${
            x + radius * xDir
        },${y}`;
    }
 ...
 */
export function getLabelPosition(points, minGap = 20) {
 ...
 */
export function areLinesPerpendicular(p1, p2, p3, p4) {
 ...
 */
export function areLinesParallel(p1, p2, p3, p4) {
 ...
 */
export function areLinesSameDirection(p1, p2, p3, p4) {
    return (
 ...
 */
export function areLinesReverseDirection(p1, p2, p3, p4) {
    return (
        (p1.x === p2.x && p3.x === p4.x && (p1.y - p2.y) * (p3.y - p4.y) < 0) ||
        (p1.y === p2.y && p3.y === p4.y && (p1.x - p2.x) * (p3.x - p4.x) < 0)
    );
...
export function getAngleBetweenLines(p1, p2, p3, p4) {
